...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            ...
            long dashCount =  s.chars().filter(ch -> ch == '-').count();
            long underScoreCount = s.chars().filter(ch -> ch == '_').count();
            if (underScoreCount == 0 || dashCount < 2) {
                pw.println(0);
                continue;
            }
            pw.println((dashCount / 2) * ((dashCount + 1) / 2) * (n - dashCount));
        }
        pw.flush();
        pw.close();
        br.close();
